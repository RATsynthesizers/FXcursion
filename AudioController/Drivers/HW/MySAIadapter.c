...
IN_RAM_D1_DMA SAIadapter_TypeDef sai2adapter;
...
void saiAdapter_Init(SAIadapter_TypeDef *adapter, HW_SAI_Instance sai_instatnce) {
	...
	if (sai_instatnce == HW_SAI1) {
		HAL_SAI_Receive_DMA(&hsai_BlockA1, (uint8_t*) adapter->RxData,
				SAI_IO_BUF_SIZE);
		HAL_SAI_Transmit_DMA(&hsai_BlockB1, (uint8_t*) adapter->TxData,
				SAI_IO_BUF_SIZE);
	}
	if (sai_instatnce == HW_SAI2) {
		HAL_SAI_Receive_DMA(&hsai_BlockA2, (uint8_t*) adapter->RxData,
				SAI_IO_BUF_SIZE);
		HAL_SAI_Transmit_DMA(&hsai_BlockB2, (uint8_t*) adapter->TxData,
				SAI_IO_BUF_SIZE);
	}
...
//	for (int i = 0; i < AUDIOBUF_SIZE; i++) {
//		audioTransmitBuffer[i] = 0;
//	}
	__enable_irq();
}
...
void HAL_SAI_TxHalfCpltCallback(SAI_HandleTypeDef *hsai) {
	if (hsai == &hsai_BlockB1)
		sai1adapter.TxHalfFlag = 1;			//++;

	if (hsai == &hsai_BlockB2) {
		sai2adapter.TxHalfFlag = 1;
//		for (int i = 0; i < SAI_IO_BUF_SIZE; i++)
//			audioTransmitBuffer[i] = sai2adapter.TxData[i];

		HAL_SPI_Transmit_DMA(&hspi2, (uint8_t*) sai2adapter.TxData, SAI_IO_BUF_SIZE / 2);
	}
}

void HAL_SAI_TxCpltCallback(SAI_HandleTypeDef *hsai) {
	if (hsai == &hsai_BlockB1)
		sai1adapter.TxFlag = 1;			//++;

	if (hsai == &hsai_BlockB2) {
		sai2adapter.TxFlag = 1;
//		for (int i = 0; i < SAI_IO_BUF_SIZE; i++)
//			audioTransmitBuffer[i] = sai2adapter.TxData[i];

		HAL_SPI_Transmit_DMA(&hspi2, (uint8_t*) sai2adapter.TxData + SAI_IO_BUF_SIZE / 2, SAI_IO_BUF_SIZE / 2);
	}
}
