...
void AudioSys::update(void) {
	...
	if (status == osOK) {
		...
		case MODULE_REVERB:
			...
			case PARAM_DECAY:
				newValue = modules[1]->p[modules[1]->paramNum - 1].val + updateAudioParams_msg.value;
				if(newValue < 0)
					newValue = 0;
				else if(newValue > 1)
					newValue = 1;
				...
				osMessageQueuePut(GaugeValuesQueueHandle, &updateGaugeValues_msg, 0, 0);
				break;
			case PARAM_REVMIX:
				newValue = modules[1]->p[modules[1]->paramNum - 2].val + updateAudioParams_msg.value;
				if(newValue < 0)
					newValue = 0;
				else if(newValue > 1)
					newValue = 1;
	...
	if (saiAdapterCheckUpdate(adapter1)) {
