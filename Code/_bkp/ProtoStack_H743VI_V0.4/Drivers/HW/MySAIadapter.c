...
void saiAdapter_Init(SAIadapter_TypeDef* adapter, HW_SAI_Instance sai_instatnce) {
	// enable SAI rx tx (sai1 A-rx, B-tx, sai2 A-tx, B-rx)
	...
	if(sai_instatnce == HW_SAI2) {
		HAL_SAI_Receive_DMA(&hsai_BlockB2, (uint8_t *) adapter->RxData, SAI_IO_BUF_SIZE);
		HAL_SAI_Transmit_DMA(&hsai_BlockA2, (uint8_t *) adapter->TxData, SAI_IO_BUF_SIZE);
	}
