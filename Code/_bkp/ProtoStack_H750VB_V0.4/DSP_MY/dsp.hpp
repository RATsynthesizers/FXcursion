...
#define DELAY_DELAYLEN     1000
#define ALLPASS_DELAYLEN   1024
...
////// TEST ALLPASS WITH PSRAM BUFFER ///////////////////
#include "../Drivers/HW/LY68L6400.h"  // add PSRAM functionality
class AllPass_PSRAM {
public:
	float out;
	...
	AllPass_PSRAM() {
        index1 = 0;
        index2 = ALLPASS_DELAYLEN-1; // TODO ??
        buffer_PSRAM_addr = 0x000000;
        // fill psram buffer with zeros
        for(uint32_t i = 0; i < MAX_READ_CHUNK_SIZE/4; i++)
        	psram.fakeTxBuf[i+1] = 0;
        for(uint32_t i = 0; i < ALLPASS_DELAYLEN/(MAX_READ_CHUNK_SIZE/4); i++) {
        	PsramWrite(&psram, psram.fakeTxBuf+1, MAX_READ_CHUNK_SIZE/4, i*MAX_READ_CHUNK_SIZE);
        }
	...
	AllPass_PSRAM(float length, float feedback, uint32_t psram_addr) {
        index1 = 0;
        index2 = ALLPASS_DELAYLEN-1; // TODO ??
        buffer_PSRAM_addr = psram_addr;
        // fill psram buffer with zeros
        for(uint32_t i = 0; i < MAX_READ_CHUNK_SIZE/4; i++)
        	psram.fakeTxBuf[i+1] = 0xFFFF;
        for(uint32_t i = 0; i < ALLPASS_DELAYLEN/(MAX_READ_CHUNK_SIZE/4); i++) {
        	PsramWrite(&psram, psram.fakeTxBuf+1, MAX_READ_CHUNK_SIZE/4, i*MAX_READ_CHUNK_SIZE);
        }
    ...
    void process(float in);
    // psram
    uint8_t dma_tx_ready_flag;
    uint8_t dma_rx_ready_flag;
    void setPsramAddrs(uint32_t psram_addr) { this->buffer_PSRAM_addr = psram_addr; } // set addrs for all allpass filters in psram

    // now public
    float buffer_index1, buffer_index2, buffer_indexFrac;
    int index1, index2, intlength;
    uint32_t buffer_PSRAM_addr;	// 24-bit PSRAM addr for buffer
protected:
