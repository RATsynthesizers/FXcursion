...
/*
  lcdWriteCommand(0xCF); // no
  lcdWriteData(0x00);
  lcdWriteData(0x83); // c1
  lcdWriteData(0x30);

  lcdWriteCommand(0xED); // ok
  lcdWriteData(0x64);
  lcdWriteData(0x03);
  lcdWriteData(0x12);
  lcdWriteData(0x81);

  lcdWriteCommand(0xE8); // no
  lcdWriteData(0x85);
  lcdWriteData(0x01); // 00
  lcdWriteData(0x79); // 78

  lcdWriteCommand(0xCB); // ok
  lcdWriteData(0x39);
  lcdWriteData(0x2C);
  lcdWriteData(0x00);
  lcdWriteData(0x34);
  ...
  lcdWriteCommand(0xF7); // ok
  lcdWriteData(0x20);

  lcdWriteCommand(0xEA); // ok
  lcdWriteData(0x00);
  lcdWriteData(0x00);

  lcdWriteCommand(ILI9341_POWERCONTROL1); // no
  lcdWriteData(0x26); // 10

  lcdWriteCommand(ILI9341_POWERCONTROL2); // no
  lcdWriteData(0x11); // 10

  lcdWriteCommand(ILI9341_VCOMCONTROL1); // no
  lcdWriteData(0x35); // 3e
  lcdWriteData(0x3E); // 28

  lcdWriteCommand(ILI9341_VCOMCONTROL2); // no
  lcdWriteData(0xBE); // 86

  lcdWriteCommand(ILI9341_MEMCONTROL); //
  lcdWriteData(lcdPortraitConfig);

  lcdWriteCommand(ILI9341_PIXELFORMAT); // ok
  lcdWriteData(0x55);

  lcdWriteCommand(ILI9341_FRAMECONTROLNORMAL); // no
  lcdWriteData(0x00);
  lcdWriteData(0x1B); // 18 (79fps)

  lcdWriteCommand(0xF2); // no
  lcdWriteData(0x08); // 00

  lcdWriteCommand(ILI9341_GAMMASET); // ok
  lcdWriteData(0x01);

  lcdWriteCommand(ILI9341_POSITIVEGAMMCORR); // no
  lcdWriteData(0x1F);
  lcdWriteData(0x1A);
  lcdWriteData(0x18);
  lcdWriteData(0x0A);
  ...
  lcdWriteCommand(ILI9341_NEGATIVEGAMMCORR); // no
  lcdWriteData(0x00);
  lcdWriteData(0x25);
  lcdWriteData(0x27);
  lcdWriteData(0x05);
  ...
  lcdWriteCommand(ILI9341_ENTRYMODE); // nono
  lcdWriteData(0x07);

  lcdWriteCommand(ILI9341_DISPLAYFUNC); //no
...
/*	ili9341_DelayMicro(10);
	...
	lcdWriteData(0x0A); // 08
	lcdWriteData(0x82); //
	lcdWriteData(0x27);     // 320 rows
	ili9341_DelayMicro(10);
	lcdWriteCommand(0xF2);  // Enable 3G (what is it?)
	lcdWriteData(0x00);     // Turn it off
	ili9341_DelayMicro(10);
	lcdWriteCommand(0x26);  // Gamma set
	lcdWriteData(0x01);     // Gamma Curve (G2.2)
	ili9341_DelayMicro(10);
