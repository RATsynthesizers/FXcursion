...
void FXchainView::btnYES_action(void) {
	if (isSelectingEffect) {
		...
		case 1:
			...
			audioAdapter.TxBuf[0] = FX_MOVE_UP;
			audioAdapter.TxBuf[1] = channelNum;
			audioAdapter.TxBuf[2] = currentPos;
			audioAdapter.TxBuf[3] = 0; // no matter, doesn't count
			audioAdapter.TxBuf[4] = 0; // no matter, doesn't count
		...
		case 2:
			...
			audioAdapter.TxBuf[0] = FX_MOVE_DOWN;
			audioAdapter.TxBuf[1] = channelNum;
			audioAdapter.TxBuf[2] = currentPos;
			audioAdapter.TxBuf[3] = 0; // no matter, doesn't count
			audioAdapter.TxBuf[4] = 0; // no matter, doesn't count
		...
		case 3:
			if (subMenu.getIsDeletingEffect()) {
				...
				audioAdapter.TxBuf[0] = FX_DELETE;
				audioAdapter.TxBuf[1] = channelNum;
				audioAdapter.TxBuf[2] = currentPos;
				audioAdapter.TxBuf[3] = 0; // no matter, doesn't count
				audioAdapter.TxBuf[4] = 0; // no matter, doesn't count
