...
#define LCD_RST_ACTIVE HAL_GPIO_WritePin(MY_LCD_RESET_GPIO_Port, MY_LCD_RESET_Pin,   GPIO_PIN_RESET);
#define LCD_RST_IDLE   HAL_GPIO_WritePin(MY_LCD_RESET_GPIO_Port, MY_LCD_RESET_Pin,   GPIO_PIN_SET);
#define LCD_BASE0        		((uint32_t)0x60000000)
#define LCD_BASE1        		((uint32_t)0x60800000)  // A22 -> bit23? Why?
...
#define	LCD_StatusRead()		*(volatile uint16_t *) (LCD_BASE1) //if use read  Mcu interface DB0~DB15 needs increase pull high
...
typedef enum
{
	...
	LCD_ORIENTATION_LANDSCAPE_ROTATED 	= 4
