...
void saiAdapter_Init(SAIadapter_TypeDef *adapter, HW_SAI_Instance sai_instatnce) {
	...
	if (sai_instatnce == HW_SAI4) {
		HAL_SAI_Receive_DMA(&hsai_BlockA4, (uint8_t*) adapter->RxData,
				SAI_IO_BUF_SIZE);
		HAL_SAI_Transmit_DMA(&hsai_BlockB4, (uint8_t*) adapter->TxData,
				SAI_IO_BUF_SIZE);
	}
...
void HAL_SAI_TxHalfCpltCallback(SAI_HandleTypeDef *hsai) {
	...
	if (hsai == &hsai_BlockB4) {
		sai4adapter.TxHalfFlag = 1;
//		for (int i = 0; i < SAI_IO_BUF_SIZE; i++)
