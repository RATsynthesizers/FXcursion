...
void Test_RecStream(void)
{
    ...
    /* A width outside the range is a configuration fault, not a runtime
       ...
       recorder channels and clamping up would run off the staging buffer. */
    CHECK(RecStream_Stage(aSrc, (U16)AUDIO_BLOCK_FRAMES,
                          (U8)(REC_SLOT_QTY - 1U)) == (U8)REC_STAGE_NONE);
    CHECK(RecStream_Stage(aSrc, (U16)AUDIO_BLOCK_FRAMES,
                          (U8)(REC_STREAM_SLOT_MAX + 1U)) == (U8)REC_STAGE_NONE);
    CHECK_EQ_U32(RecStream_Stats()->nBlocksDropped, 0UL);
