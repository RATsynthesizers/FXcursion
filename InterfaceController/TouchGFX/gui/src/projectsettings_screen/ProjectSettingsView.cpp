...
void ProjectSettingsView::btnYES_action(void) {
	...
	if (connectDelete == NO_ACTION) {
		...
		if (connectDelete == CONNECT) {
			audioAdapter.TxBuf[0] = CHAIN_PLUG_WIRE;
			audioAdapter.TxBuf[1] = selectedLineNumber;
			audioAdapter.TxBuf[2] = 0; // no matter, doesn't count
			audioAdapter.TxBuf[3] = 0; // no matter, doesn't count
			audioAdapter.TxBuf[4] = 0; // no matter, doesn't count

			HAL_UART_Transmit_DMA(&huart2, audioAdapter.TxBuf,
					AUDIOUART_OUTBUF_SIZE);
			...
			audioAdapter.TxBuf[0] = CHAIN_UNPLUG_WIRE;
			audioAdapter.TxBuf[1] = selectedLineNumber;
			audioAdapter.TxBuf[2] = 0; // no matter, doesn't count
			audioAdapter.TxBuf[3] = 0; // no matter, doesn't count
			audioAdapter.TxBuf[4] = 0; // no matter, doesn't count
